...
class StationImageController extends Controller
{
    ...
    public function upload(Request $request, $id)
    {
        if($request->hasFile('file'))
        {
            ...
            imagedestroy($src);
            imagejpeg($dst,base_path('/../progress/'.$folder->station()->code.'/'.$folder->name.'/').preg_replace('/\s+/', '', $imageName)); // adjust format as needed
            imagedestroy($dst);
            // $imageFile->move(base_path('/../progress/'.$folder->name), preg_replace('/\s+/', '', $imageName));


            $file_info = pathinfo(base_path('/../progress/'.$folder->station()->code.'/'.$folder->name.'/'.$imageName));
            ...
            if(strtolower($file_info['extension']) == "jpg" or strtolower($file_info['extension']) == "jpeg" or strtolower($file_info['extension']) == "png"){
                ...
                if(strtolower($file_info['extension']) == "zip" or strtolower($file_info['extension']) == "rar"){
                    $zip = new ZipArchive;
                    $res = $zip->open(base_path('/../progress/'.$folder->station()->code.'/'.$folder->name.'/'.$imageName));
                    if ($res === TRUE) {
                        // extract it to the path we determined above
                        $zip->extractTo(base_path('/../progress/'.$folder->station()->code.'/'.$folder->name));
                        $zip->close();
                        $all_images = ConstructImage::where('folder_id', $id)->get();
                        foreach(glob(base_path('/../progress/'.$folder->station()->code.'/'.$folder->name.'/*.*')) as $filename){
                            foreach ($all_images as $all_image){
                                if($filename != $all_image->name){
                                    $newname = uniqid().preg_replace('/\s+/', '', $filename);
                                    rename(base_path('/../progress/'.$folder->station()->code.'/'.$folder->name.'/'.$filename), $newname);
                                    $image = new ConstructImage();
                                    $image->folder_id = $id;
                                    $image->name = $newname;
                                    $image->save();
