                        ...
                        @endif
                            @if (sizeof($errors) != 0)
                                <div class="alert alert-danger" role="alert">
                                    ...
                                    @if($errors->has('permission')) จำเป็นต้องกำหนดสิทธิ์การเข้าถึง @endif

                                </div>

                            @endif
                        <p class="small hint-text">* Indicates a required field.</p>
                            {{ Form::model($role, array('route' => array('roles.update', $role->id), 'method' => 'PUT', 'id' => 'form-work', 'class' => 'form-horizontal', 'role'=>'form', 'autocomplete'=>'off','novalidate'=>'novalidate')) }}{{-- Form model binding to automatically populate our fields with permission data --}}
                            {{csrf_field()}}
                            <div class="form-group row{{ $errors->has('name') ? ' has-error' : '' }}">
                                <label for="name" class="col-md-3 control-label">ชื่อ *</label>
                                <div class="col-md-9">
                                    <input type="text" class="form-control error" id="name" placeholder="Administrator" name="name"  value="{{old('name',$role->name)}}" required="" aria-required="true" aria-invalid="true">
                                </div>
                            </div>
